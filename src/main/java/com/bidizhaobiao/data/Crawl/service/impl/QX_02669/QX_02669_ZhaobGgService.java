 ...
 * 原网站：http://bm.zhangpu-ks.cn/gslist.aspx?key=%u62db%u6807%u516c%u544a
 * 主页：http://bm.zhangpu-ks.cn
 **/

@Service("QX_02669_ZhaobGgService")
public class QX_02669_ZhaobGgService extends SpiderService implements PageProcessor {

    public Spider spider = null;

    public String listUrl = "http://bm.zhangpu-ks.cn/gsList.aspx?key=%E9%87%87%E8%B4%AD%E5%85%AC%E5%91%8A&type=zf";
    ...
    public void process(Page page) {
        ...
        try {
            ...
            if (url.contains("key")) {
                ...
                Elements lis = document.select("ul#ul_cggg li:has(a)");
                if (lis.size() > 0) {
                    for (Element li : lis) {
                ...
                String content = "";
                Document document = Jsoup.parse(page.getRawText().replace("&nbsp;", "").replace("&amp;", "&").replace("&ensp;", "").replace("<![CDATA[", "").replace("]]>", "").replace("&lt;", "<").replace("&gt;", ">"));
                Element contentE = document.select("div.boxForm").first();
    ...
    public Request getRequest(int pageIndex, String __VIEWSTATE, String __VIEWSTATEGENERATOR) {
        Request request = new Request("http://bm.zhangpu-ks.cn/gsList.aspx?key=%E9%87%87%E8%B4%AD%E5%85%AC%E5%91%8A&type=zf");
