 ...
 * 原网站：http://www.myx.gov.cn/list.html?channel=district&pages=29
 * 主页：http://www.myx.gov.cn
 **/

@Service("QX_05614_2_ZhongbXxService")
public class QX_05614_2_ZhongbXxService extends SpiderService implements PageProcessor {

    public Spider spider = null;

    public String listUrl = "http://api.myx.gov.cn/news?category=29&page=0&limit=15";

    public String baseUrl = "http://www.myx.gov.cn";
    // 网站编号
    public String sourceNum = "05614-2";
    // 网站名称
        ...
        try {
            ...
            if (url.contains("page")) {
                    ...
                    if (jsonArray.length() > 0) {
                        for (int i = 0; i < jsonArray.length(); i++) {
                           ...
                           /* if (a.hasAttr("title")) {
                            ...
                            String id = href;
                            //http://www.myx.gov.cn/detail.html?did=
                            String detailLink = "http://www.myx.gov.cn/detail.html?did=" + href;
                ...
                if (serviceContext.getPageNum() == 1) {
                    String pages = jsonObject.get("pages").toString().trim();
                    int maxPage = Integer.parseInt(pages);
                    serviceContext.setMaxPage(maxPage);
                }
                if (serviceContext.getPageNum() < serviceContext.getMaxPage() && serviceContext.isNeedCrawl()) {
                    serviceContext.setPageNum(serviceContext.getPageNum() + 1);
                    page.addTargetRequest(listUrl.replace("page=0", "page=" + (serviceContext.getPageNum() - 1)));
                }
            } else {
                // 列表页请求
