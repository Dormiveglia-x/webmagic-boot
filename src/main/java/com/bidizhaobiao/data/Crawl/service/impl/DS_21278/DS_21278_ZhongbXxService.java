package com.bidizhaobiao.data.Crawl.service.impl.DS_21278;
 ...
 * 原网站：http://www.whjgtx.cn/news/3/
 * 主页：http://www.whjgtx.cn
 **/
@Service
public class DS_21278_ZhongbXxService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://www.whjgtx.cn/news/3/#c_portalResNews_list-16360982923089774-1";
    public String baseUrl = "http://www.whjgtx.cn";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "21278";
    // 网站名称
    public String sourceName = "威海激光通信先进技术研究院";
    // 信息源
    ...
    public void process(Page page) {
        ...
        try {
            ...
            if (url.contains("c_portalResNews_list")) {
                ...
                Elements listElement = doc.select("div.p_news>div:has(a)");
                if (listElement.size() > 0) {
                    for (Element element : listElement) {
                        ...
                        String date = "";
                        Matcher dateMat = datePat.matcher(element.select("span.time").text());
                        if (dateMat.find()) {
                        ...
                        if (title.length() < 2) title = a.select("span.title").text().trim();
                ...
                if (serviceContext.getPageNum() == 1) {
                    Pattern maxPat = Pattern.compile("totalPage:\"(\\d+)\"");
                    Matcher maxMat = maxPat.matcher(doc.outerHtml());
                    if (maxMat.find()) {
                        int maxPage = Integer.parseInt(maxMat.group(1));
                        serviceContext.setMaxPage(maxPage);
                    }
                }
                if (serviceContext.getPageNum() < serviceContext.getMaxPage() && serviceContext.isNeedCrawl()) {
                    serviceContext.setPageNum(serviceContext.getPageNum() + 1);
                    String href = listUrl.replace("list-16360982923089774-1", "list-16360982923089774-" + serviceContext.getPageNum());
                ...
                if (branch != null) {
                    ...
                    String content = "";
                    Element contentElement = doc.select("div.p_articles").first();
                    if (contentElement != null) {
                        ...
                        Element titleElement = doc.select("h1.e_title>div").first();
